...
WINDOW_TITLE = "Miracle 7.4 - Dropper"
...
class WindowScreenshot:
    ...
    def get_screenshot(self, region=None, dimanesion=True):
        ...
        if not self.bmpinfo or not self.bmpstr:
            print("Error: Data captured is not valid or window minimized.")
        ...
        try:
            ...
            print(f"Error in capture process screen: {e}")
            return None
...
def capture_and_save_screenshot(filename="tela.png"):
    # Captura uma captura de tela da janela
    with WindowScreenshot() as screenshot:
